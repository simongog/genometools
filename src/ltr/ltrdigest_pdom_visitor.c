...
static void gt_hmmer_parse_model_hit_delete(GtHMMERModelHit *mh);
...
static GtHMMERParseStatus* gt_hmmer_parse_status_new(void)
{
  ...
  s->models = gt_hashmap_new(GT_HASH_STRING, gt_free_func,
                             (GtFree) gt_hmmer_parse_model_hit_delete);
  return s;
}
...
GtRange gt_ltrdigest_pdom_visitor_coords(GtLTRdigestPdomVisitor *lv,
                                         const GtHMMERSingleHit *singlehit)
{
  ...
  switch (singlehit->strand)
  {
    ...
    default:
      retrng.start = lv->leftLTR_5 + (singlehit->alifrom) * GT_CODON_LENGTH
                        + (unsigned long) singlehit->frame;
      retrng.end   =  retrng.start + (singlehit->alito - singlehit->alifrom)
                           * GT_CODON_LENGTH;
      break;
    case GT_STRAND_REVERSE:
      retrng.start = lv->rightLTR_3 - (singlehit->alito + 1) * GT_CODON_LENGTH
                        - (unsigned long) singlehit->frame;
      retrng.end   =  retrng.start + (singlehit->alito - singlehit->alifrom)
                           * GT_CODON_LENGTH;
      break;
                                                  ...
                                                  GtError *err)
{
  ...
  if (!had_err && lv->ltr_retrotrans != NULL) {
    ...
    if (!had_err) {
      ...
      rval = pipe(cp);
      gt_assert(rval == 0);
      args = gt_cstr_split(gt_str_get(lv->cmdline), ' ');
      switch ((pid = (int) fork())) {
        case -1:
          perror("Can't fork");
        ...
        case 0:    /* child */
          ...
          (void) close(pc[1]);
          (void) close(cp[0]);
          (void) execvp("hmmscan", args); /* XXX: read hmmer path from env */
          perror("couldn't execute hmmscan!");  /* XXX */
          exit(1);
        default:    /* parent */
          for (i = 0UL; i < 3UL; i++) {
            char buf[5];
